...
public class ProjectSecurityConfig {
  ...
  SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http, MvcRequestMatcher.Builder mvc) throws Exception {
    ...
    http.csrf((csrf) -> csrf.ignoringRequestMatchers("/saveMsg","/public/**","/api/**"
            ,"/data-api/**","/eazyschool/actuator/**")) // Enable CSRF protection for all webpages except for ...
//            .ignoringRequestMatchers("/h2-console/**")) // Enable CSRF protection for all webpages except for h2-console
        ...
        .authorizeHttpRequests((requests) -> requests
            ...
            .requestMatchers(mvc.pattern("/data-api/**")).authenticated()
            .requestMatchers(mvc.pattern("/eazyschool/actuator/**")).hasRole("ADMIN") // Actuator for admins only
...
//            .requestMatchers(mvc.pattern("/data-api/**")).permitAll() // HAL Explorer
