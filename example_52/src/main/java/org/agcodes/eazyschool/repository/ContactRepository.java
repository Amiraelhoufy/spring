...
//  @Query("SELECT c from Contact c WHERE c.status = :status")
  ...
  /**** Note: @Modifying @Query	won't trigger updatedAt / updatedBy ****/
 @Transactional
 @Modifying
...
// @Query(value = "UPDATE contact_msg c SET c.status = :status WHERE contact_id= :contactId",nativeQuery = true)
 int updateStatusById(@Param("contactId") int contactId,@Param("status") String status/*, @Param("updatedBy") String updatedBy*/);
